...
1. **도시 검색** — Open-Meteo Geocoding
   ...
   **`language=en` 폴백은 쓰지 않는다(2026-08-06 실측으로 폐기).** `language`는
   결과의 **표시 언어만** 바꾸고 검색어 매칭에는 관여하지 않는다. 확인한 결과:

   | 질의 | language | 결과 |
   |---|---|---|
   | `다낭` | ko | 0건 |
   | `다낭` | en | 0건 — 폴백해도 그대로 0건이다 |
   | `Da Nang` | ko | 3건, 이름은 `다낭 / 베트남`으로 한국어 표시 |
   | `오사카` / `방콕` | ko | 정상 |

   즉 한국어 이름은 **표시용으로만 있고 검색 색인에는 없다.** 색인 보유 여부가
   도시마다 들쭉날쭉해(오사카·방콕 O, 다낭 X) 한국어 사용자가 0건을 만나는 일이
   흔하다. 그러므로:
   - 조회는 항상 `language=ko` 하나로 한다(맞으면 결과가 한국어로 나온다)
   - 0건이면 재조회하지 말고 **영어·현지 이름으로 다시 검색하도록 안내**한다
     (예: `다낭 → Da Nang`). 없는 폴백을 도는 것보다 정직하고 빠르다
