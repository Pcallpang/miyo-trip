...
├ 여행 상세  #/t/<id>      지금 화면 그대로 (요약/하단 탭/타임라인/고정섹션)
│    항목을 탭하면 편집 모달, 추가 버튼은 항상 표시 (편집 토글 없음)
└ 여행 설정  #/t/<id>/edit 제목·기간·도시·통화·인원·숙소·섹션
```

여행이 하나뿐이면 목록을 건너뛰고 바로 상세로 간다 — 지금과 동일한 사용감을 유지한다.

**편집 토글은 두지 않는다.** 처음에는 일정·섹션·결제내역에 각각 편집 모드 토글을
뒀지만 걷어냈다. 이 앱은 내 여행을 내가 고치는 것뿐이라 "보는 사람"과 "고치는 사람"이
갈리지 않고, 입력이 전부 모달로 옮겨간 뒤로는 폼이 화면을 어지럽히지도 않는다.
삭제는 편집 모달 안에 둔다 — 목록에 삭제 버튼을 늘어놓으면 읽을 때 복잡하고 폰에서
잘못 누르기 쉽다.

탭 대상을 `<button>`으로 만들 수 없는 두 곳은 예외를 둔다: 결제 내역은 표라
`<tr>`에 `role="button"`/`tabindex`를 주고 Enter·Space를 직접 받고, 섹션은
`<details>`라 제목 탭이 이미 펼치기이므로 펼친 내용 아래에 도구줄(↑ ↓ 수정)을 둔다.
...
1. **도시 검색** — Open-Meteo Geocoding
   ...
   **`language=en` 폴백은 쓰지 않는다(2026-08-06 브라우저 실측).** `language`는
   표시 언어뿐 아니라 **검색 색인 자체를 고른다.** 측정 결과:

   | 질의 | `language=ko` | `language=en` |
   |---|---|---|
   | 다낭·호이안·오사카·방콕·타이베이·세부·하노이·후쿠오카 | **8/8 성공**, 한국어 표시 | **8/8 0건** |
   | Da Nang·Osaka·Bangkok·Hoi An | **4/4 성공**, 한국어 표시 | 4/4 성공, 영어 표시 |

   즉 `language=ko`는 **한국어 질의와 영어 질의를 모두** 받으면서 결과를 한국어로
   돌려준다. 반대로 `language=en`은 한국어 질의를 전부 죽인다. 그러므로 조회는
   `language=ko` 하나로 끝내고 폴백을 두지 않는다 — 폴백은 이득이 없을 뿐 아니라
   한국어 사용자에게 해롭다.

   > 주의: 이 표는 앱과 같은 실행 환경(브라우저 `fetch`)에서 측정한 값이다. 같은
   > URL을 다른 도구로 받아 본 1차 측정에서는 한국어 질의가 전부 0건으로 나왔는데,
   > 그 도구가 한국어 질의를 제대로 전달하지 못한 탓이었다. **지오코딩 동작을
   > 확인할 때는 반드시 브라우저에서 잰다.**
